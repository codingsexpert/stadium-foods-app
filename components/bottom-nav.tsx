...
export function BottomNav({ onOpenCart, onOpenMap, onOpenLeaderboard, onOpenOrders }: BottomNavProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-around px-1 py-1 pb-[env(safe-area-inset-bottom,8px)]">
        {navItems.map((item) => {
              ...
              <div className="relative">
                ...
                {item.badge && item.badge > 0 && (
                  <motion.span 
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                    className="absolute -top-1.5 -right-2 min-w-[18px] h-[18px] px-1 rounded-full bg-primary text-primary-foreground text-[10px] flex items-center justify-center font-bold"
                  >
